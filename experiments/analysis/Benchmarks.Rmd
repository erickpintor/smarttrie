...
load_checkpoint_marks = function(server_log_path,
                                 start_ts,
                                 end_ts,
                                 version = 1) {
  if (version == 0) {
...
plot_gc_table = function(...) {
  ...
  ggtexttable(df, rows = NULL, theme = ttheme("light")) %>%
    tab_add_footnote(text = "Unit: requests per second.",
                     size = 9,
                     face = "italic")
}
...
```{r 2021-01-31-gc-analysis, echo=FALSE, message=FALSE, warning=FALSE}
fig # work arround blank plot issue
```

### Vazão média em relação ao modelo de concorrência

```{r experiment-2021-01-31-blocking-vs-non-blocking, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r experiment-2021-01-31-throughput-break-down, echo=FALSE, message=FALSE, warning=FALSE}
...
for (model in list(
  list(state = "tree-map", label = "Blocking"),
  list(state = "trie-map", label = "Concurrent")
)) {
  for (th in c("001",
               "002",
               "004",
    ...
    path = paste0(basepath, model$state, "/", th, benchpath)
    
    ycsb =
      load_ycsb_throughput(path) %>%
      summarise_at(vars(throughput, p99), mean) %>%
      mutate(
        threads = as.factor(as.numeric(th)),
        model = as.factor(model$label),
        p99 = p99 / 1000
      ) # us to ms
...
ggplot(benchmarks, aes(threads, throughput, group = model, color = model)) +
  ...
  labs(title = "Throughput by number of threads", x = "Threads", y = "Throughput (rps)") +
...
ggplot(benchmarks, aes(threads, p99, group = model, color = model)) +
  geom_point() +  geom_line() +
  labs(title = "Latency by number of threads", x = "Threads", y = "Throughput (rps)") +
  scale_color_discrete(name = "Model") +
  scale_y_continuous(breaks = pretty_breaks(15))
```

```{r system-saturation-point-latency-by-throughput, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(benchmarks, aes(throughput, p99, color = model)) +
  geom_path() + geom_point(aes(shape = threads), size = 3) +
...
```{r experiment-2020-02-27-tree-map, echo=FALSE, message=FALSE, warning=FALSE}
plot_throughput_vs_checkpoints(
  "TreeMap",
  "../results/20210227/tree-map/client/tmp/SmartTrie/logs/benchmark.log",
  "../results/20210227/tree-map/server/tmp/SmartTrie/logs/app.log",
  # "../results/20210227/tree-map/server/tmp/SmartTrie/logs/gc.log"
) + 
  scale_x_datetime(breaks = pretty_breaks(10)) +
  coord_cartesian(xlim=c(as.POSIXct("2021-02-27 17:50:30"),
                         as.POSIXct("2021-02-27 18:44:00")))
```

```{r experiment-2020-02-27-trie-map, echo=FALSE, message=FALSE, warning=FALSE}
plot_throughput_vs_checkpoints(
  "TrieMap",
  "../results/20210227/trie-map/client/tmp/SmartTrie/logs/benchmark.log",
  "../results/20210227/trie-map/server/tmp/SmartTrie/logs/app.log",
  # "../results/20210227/trie-map/server/tmp/SmartTrie/logs/gc.log"
) + 
  scale_x_datetime(breaks = pretty_breaks(10)) +
  coord_cartesian(xlim=c(as.POSIXct("2021-02-27 19:10:00"),
                         as.POSIXct("2021-02-27 19:54:00")))
```

### Vazão média em relação ao modelo de concorrência

```{r experiment-2021-02-27-blocking-vs-non-blocking, echo=FALSE, message=FALSE, warning=FALSE}
plot_blocking_vs_non_blocking = function () {
  tree_map = load_ycsb_throughput("../results/20210227/tree-map/client/tmp/SmartTrie/logs/benchmark.log")
  trie_map = load_ycsb_throughput("../results/20210227/trie-map/client/tmp/SmartTrie/logs/benchmark.log")
...
```{r experiment-2021-02-27-throughput-break-down, echo=FALSE, message=FALSE, warning=FALSE}
plot_non_blocking_breakdown = function () {
  bench = load_ycsb_throughput("../results/20210227/trie-map/client/tmp/SmartTrie/logs/benchmark.log")
  
  cout = 
    bench %>% 
			filter(ts < as.POSIXct("2021-02-27 19:01:07") | ts > as.POSIXct("2021-02-27 19:01:55")) %>%
			filter(ts < as.POSIXct("2021-02-27 19:05:59") | ts > as.POSIXct("2021-02-27 19:06:44")) %>%
			filter(ts < as.POSIXct("2021-02-27 19:10:50") | ts > as.POSIXct("2021-02-27 19:11:36")) %>%
			filter(ts < as.POSIXct("2021-02-27 19:15:43") | ts > as.POSIXct("2021-02-27 19:16:29")) %>%
			filter(ts < as.POSIXct("2021-02-27 19:20:34") | ts > as.POSIXct("2021-02-27 19:21:20")) %>%
			filter(ts < as.POSIXct("2021-02-27 19:25:21") | ts > as.POSIXct("2021-02-27 19:26:07")) %>%
			filter(ts < as.POSIXct("2021-02-27 19:30:11") | ts > as.POSIXct("2021-02-27 19:30:56")) %>%
			filter(ts < as.POSIXct("2021-02-27 19:35:01") | ts > as.POSIXct("2021-02-27 19:35:47")) %>%
			filter(ts < as.POSIXct("2021-02-27 19:39:52") | ts > as.POSIXct("2021-02-27 19:40:38")) %>%
			filter(ts < as.POSIXct("2021-02-27 19:44:42") | ts > as.POSIXct("2021-02-27 19:45:28")) %>%
			filter(ts < as.POSIXct("2021-02-27 19:49:31") | ts > as.POSIXct("2021-02-27 19:50:17")) %>%
			filter(ts < as.POSIXct("2021-02-27 19:54:22") | ts > as.POSIXct("2021-02-27 19:55:08"))
  
  cin =
    bench %>% 
      filter(
				(ts >= as.POSIXct("2021-02-27 19:01:07") & ts <= as.POSIXct("2021-02-27 19:01:55")) |
				(ts >= as.POSIXct("2021-02-27 19:05:59") & ts <= as.POSIXct("2021-02-27 19:06:44")) |
				(ts >= as.POSIXct("2021-02-27 19:10:50") & ts <= as.POSIXct("2021-02-27 19:11:36")) |
				(ts >= as.POSIXct("2021-02-27 19:15:43") & ts <= as.POSIXct("2021-02-27 19:16:29")) |
				(ts >= as.POSIXct("2021-02-27 19:20:34") & ts <= as.POSIXct("2021-02-27 19:21:20")) |
				(ts >= as.POSIXct("2021-02-27 19:25:21") & ts <= as.POSIXct("2021-02-27 19:26:07")) |
				(ts >= as.POSIXct("2021-02-27 19:30:11") & ts <= as.POSIXct("2021-02-27 19:30:56")) |
				(ts >= as.POSIXct("2021-02-27 19:35:01") & ts <= as.POSIXct("2021-02-27 19:35:47")) |
				(ts >= as.POSIXct("2021-02-27 19:39:52") & ts <= as.POSIXct("2021-02-27 19:40:38")) |
				(ts >= as.POSIXct("2021-02-27 19:44:42") & ts <= as.POSIXct("2021-02-27 19:45:28")) |
				(ts >= as.POSIXct("2021-02-27 19:49:31") & ts <= as.POSIXct("2021-02-27 19:50:17")) |
				(ts >= as.POSIXct("2021-02-27 19:54:22") & ts <= as.POSIXct("2021-02-27 19:55:08")))
...
```{r experiment-2021-02-28-experiment-matrix, echo=FALSE, message=FALSE, warning=FALSE}
benchmarks = tibble()
checkpoints = tibble()
root = "../results/20210228/"

for (dataset in list(
  list(records = "25600",   label = "100M"),
  list(records = "262144",  label = "1G"),
  list(records = "1310720", label = "5G")
)) {
  for (model in list(
    list(state = "tree-map", label = "Blocking"),
    list(state = "trie-map", label = "Concurrent")
  )) {
    for (threads in list("1", "2", "4", "8")) {
      basepath = paste0(root, dataset$records, "/", model$state, "/", threads)
      benchpath = paste0(basepath, "/client/tmp/SmartTrie/logs/benchmark.log")
      checkpath = paste0(basepath, "/server/tmp/SmartTrie/logs/app.log")
      
      if (file.exists(benchpath)) {
        ycsb =
          load_ycsb_throughput(benchpath) %>%
          mutate(
            size = as.factor(dataset$label),
            model = as.factor(model$label),
            threads = as.factor(threads)
          )
        
        checks =
          load_checkpoint_marks(checkpath, first(ycsb$ts), last(ycsb$ts)) %>%
          mutate(
            size = as.factor(dataset$label),
            model = as.factor(model$label),
            threads = as.factor(threads)
        ...
        benchmarks = bind_rows(benchmarks, ycsb)
        checkpoints = bind_rows(checkpoints, checks)
      }
    }
  }
}
...
# ggplot(benchmarks) +
#   geom_violin(aes(model, throughput, fill = model)) +
#   facet_grid(size ~ threads, labeller = labeller(
#     threads = function (n) {
#       ifelse(n == 1, "1 thread", paste(n, "threads"))
#     }
#   )) +
#   scale_x_discrete(labels = NULL, breaks = NULL) +
#   labs(title = "Thoughput by data set and number of threads",
#        x = NULL,
#        y = "Throughput (rps)",
#        fill = "Model")

benchmarks %>%
  group_by(size, threads, model) %>%
  summarise(throughput = mean(throughput)) %>%
  ggplot() +
  geom_bar(aes(model, throughput, fill = model),
           stat = "identity",
           width = 0.7) +
  facet_grid(size ~ threads, labeller = labeller(
    threads = function (n) {
      ifelse(n == 1, "1 client", paste(n, "clients"))
    }
  )) +
  scale_x_discrete(labels = NULL, breaks = NULL) +
  labs(title = "Thoughput by data set and number of threads",
...
```{r experiment-2021-03-06-state-microbench, echo=FALSE, message=FALSE, warning=FALSE}
raw = "
   size    state       score       error
   100M  TreeMap 1839965.612 1189038.587
   100M  TrieMap 2760493.478   64591.277
     1G  TreeMap  484690.679   10343.074
     1G  TrieMap 1081895.548  189892.001
     5G  TreeMap  352065.384  109470.738
     5G  TrieMap  683722.914   14456.405
"

microbenchmarks = readr::read_delim(
  raw,
  delim = " ",
  col_types = list(
    readr::col_factor(),
...
ggplot(microbenchmarks, aes(size, score, fill = state)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_discrete(name = "Data structure") +
  scale_y_continuous(labels = number_format(big.mark = "."),
                     breaks = pretty_breaks(15)) +
  labs(title = "Throughput by data structure and size",
       x = "Size",
