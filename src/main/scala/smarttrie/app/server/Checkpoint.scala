...
trait CheckpointController {
  ...
  final def read(): Option[Checkpoint.Reader] =
...
object CheckpointController {
  ...
  def apply(
      ckpPath: Path,
      async: Boolean,
      blockSizeMB: Int = DefaultBlockSizeMB
  ): CheckpointController =
    if (async) new Async(ckpPath, blockSizeMB)
    else new Sync(ckpPath, blockSizeMB)

  private final class Async(val ckpPath: Path, val blockSizeMB: Int)
  ...
  private final class Sync(val ckpPath: Path, val blockSizeMB: Int)
      extends CheckpointController {
...
object Checkpoint {
  ...
  def write(
    ...
    val writer = new Writer(ckpPath, blockSize)
  ...
  def read(
    ...
    checkpoints.headOption map { new Reader(_, blockSize) }
  }

  final class Writer private[Checkpoint] (ckpPath: Path, blockSize: Long) {
  ...
  final class Reader private[Checkpoint] (filePath: Path, blockSize: Int) {
    import FileChannel.MapMode._
    import StandardOpenOption._
